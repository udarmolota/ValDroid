...
EXPORT int my_pthread_mutex_init(pthread_mutex_t *m, my_mutexattr_t *att)
{
	...
	*(uint64_t*)(((uintptr_t)m) + 40) = save;	// put back overwritten value. Nasty but should be fast and quite safe
	...
	int ret = pthread_mutex_init(m, att?(&mattr.nat):NULL);
	#ifdef ANDROID
	((int*)m)[4] = 0;	// never let stale guest memory look like a glibc __kind (see rd_adopt_glibc_static_mutex)
	#endif
	#else
...
#ifdef ANDROID
// ValDroid: glibc static initializers (PTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP, hence libstdc++'s
// std::recursive_mutex) store the kind at +16 and leave bionic's state word at +0 zero, so bionic
// treats the mutex as NORMAL and a recursive re-lock from the same thread deadlocks (Goldberg's
// SteamAPI_Init hung exactly there). Move the kind into bionic's state word once; after the first
// lock bionic keeps the type bits in that word, so the check never fires again for this mutex.
static inline void rd_adopt_glibc_static_mutex(pthread_mutex_t* m)
{
	uint32_t* state = (uint32_t*)m;
	int kind = ((int*)m)[4] & 3;	// glibc __kind: 1=RECURSIVE, 2=ERRORCHECK, 3=ADAPTIVE_NP
	if(kind == 3) kind = 0;
	if(kind && !__atomic_load_n(state, __ATOMIC_RELAXED)) {
		uint32_t zero = 0;	// bionic: PTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP == { (type & 3) << 14 }
		__atomic_compare_exchange_n(state, &zero, (uint32_t)kind << 14, 0, __ATOMIC_ACQ_REL, __ATOMIC_RELAXED);
	}
}
EXPORT int my_pthread_mutex_lock(pthread_mutex_t* m)
{
	rd_adopt_glibc_static_mutex(m);
