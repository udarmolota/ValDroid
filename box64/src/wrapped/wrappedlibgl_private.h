...
GOM(glXChooseFBConfig, pFEpipp)            // RimDroid: -> one dummy FBConfig (core-profile path)
GO(glXChooseFBConfigSGIX, pFpipp)
GOM(glXCreateContext, pFEpppi)             // RimDroid: -> ZFA context (RimWorld 1.6 GLX path)
GOM(glXCreateContextAttribsARB, pFEpppip)  // RimDroid: -> ZFA context
...
GOM(glXCreateNewContext, pFEppipi)         // RimDroid: -> ZFA context (FBConfig path)
GO(glXCreatePbuffer, LFppp)
GO(glXCreatePixmap, LFppLp)
GOM(glXCreateWindow, LFEppLp)              // RimDroid: -> the X window itself (identity)
GOM(glXChooseVisual, pFEpip)               // RimDroid: -> dummy visual (ZFA path)
GO(glXCopyImageSubDataNV, vFppuuiiiipuuiiiiiii)
GO(glXCopySubBufferMESA, vFpLiiii)
GOM(glXDestroyContext, vFEpp)              // RimDroid: no-op in ZFA path
...
GOM(glXDestroyWindow, vFEpL)               // RimDroid: no-op (nothing was created)
GO(glXEnumerateVideoCaptureDevicesNV, pFpip)
GO(glXEnumerateVideoDevicesNV, pFpip)
GOM(glXGetClientString, pFEpi)             // RimDroid: sane string (ZFA path; libzfa has no glX)
GOM(glXGetConfig, iFEppip)                 // RimDroid: sane values (ZFA path)
GOM(glXGetCurrentContext, pFEv)            // RimDroid: -> current ZFA handle
GOM(glXGetCurrentDisplay, pFEv)            // RimDroid: -> last display (ZFA path)
GOM(glXGetCurrentDrawable, LFEv)           // RimDroid: -> current drawable (ZFA path)
GO(glXGetCurrentReadDrawable, LFv)
GO(glXGetDrawableAttributes, vFi)
GO(glXGetDrawableAttributesSGIX, vFi)
GOM(glXGetFBConfigs, pFEpip)               // RimDroid: -> one dummy FBConfig
GOM(glXGetFBConfigAttrib, iFEppip)         // RimDroid: -> describes the dummy FBConfig
...
GOM(glXGetVisualFromFBConfig, pFEpp)       // RimDroid: -> dummy visual (ZFA path)
...
GOM(glXMakeCurrent, iFEpLp)                // RimDroid: -> zfa_make_current / release (ZFA path)
GO(glXQueryContext, iFppip)
GOM(glXQueryDrawable, vFEpLip)             // RimDroid: no-op (ZFA path)
GOM(glXQueryExtension, iFEppp)             // RimDroid: -> True (ZFA path)
GOM(glXQueryExtensionsString, pFEpi)       // RimDroid: -> "" (ZFA path)
...
GOM(glXQueryServerString, pFEpii)          // RimDroid: sane string (ZFA path)
GO(glXQuerySwapGroupNV, iFpLpp)
GOM(glXQueryVersion, iFEppp)               // RimDroid: -> 1.4 (ZFA path)
...
GOM(glXSwapBuffers, vFEpL)                 // RimDroid: -> zfa_swap present (ZFA path)
