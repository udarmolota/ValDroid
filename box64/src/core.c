...
int initialize(int argc, const char **argv, char** env, x64emu_t** emulator, elfheader_t** elfheader, int exec)
{
    ...
    *elfheader = elf_header;
    ...
    if(CalcLoadAddr(elf_header)) {
        ...
        FreeBox64Context(&my_context);
        FreeCollection(&ld_preload);
        return -21; // ValDroid diagnostic: ELF load-address calculation
    }
    ...
    if(AllocLoadElfMemory(my_context, elf_header, 1)) {
        ...
        FreeBox64Context(&my_context);
        FreeCollection(&ld_preload);
        return -22; // ValDroid diagnostic: ELF segment mapping
    }
    ...
    if(CalcStackSize(my_context)) {
        ...
        FreeBox64Context(&my_context);
        FreeCollection(&ld_preload);
        return -23; // ValDroid diagnostic: guest stack allocation
    }
    // init x86_64 emu
    ...
    if(LoadNeededLibs(elf_header, my_context->maplib, 0, 0, 0, my_context, emu)) {
        ...
        FreeBox64Context(&my_context);
        return -24; // ValDroid diagnostic: DT_NEEDED loading
    }
    // reloc...
    ...
    if(RelocateElf(my_context->maplib, NULL, 0, 0, elf_header)) {
        ...
        FreeBox64Context(&my_context);
        return -25; // ValDroid diagnostic: main ELF relocation
    }
    // and handle PLT
