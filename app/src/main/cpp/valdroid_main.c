...
//
// Built as libvaldroid_exec.so (the "lib*.so" name is the Android packaging
...
//
// Usage:  libvaldroid_exec.so <game_dir> <lib_dir> [extra RimWorld args...]
...
int main(int argc, char** argv) {
    ...
    // In the JNI app, ValDroidApplication calls System.loadLibrary("valdroidlinker"),
    // which puts libvaldroidlinker.so in the DEFAULT linker namespace so that
    ...
    void* linker = dlopen("libvaldroidlinker.so", RTLD_NOW | RTLD_GLOBAL);
    if (!linker) {
        fprintf(stderr, "rimdroid_exec: failed to load libvaldroidlinker.so: %s\n", dlerror());
        return 3;
    }
