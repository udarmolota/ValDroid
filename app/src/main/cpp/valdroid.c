...
static void launch_rimworld_elf(const char* game_dir_path, int argc, const char** argv) {
    ...
    const char* executable = getenv("VALDROID_EXECUTABLE");
    if (!executable || !executable[0]) executable = "valheim.x86_64";
    snprintf(binary_path, sizeof(binary_path), "%s/%s", game_dir_path, executable);
...
void rimdroid_start_game(const char* game_dir_path,
    ...
    const char* launch_executable = getenv("VALDROID_EXECUTABLE");
    LOGI("Starting %s via box64...",
         (launch_executable && launch_executable[0]) ? launch_executable : "valheim.x86_64");
    ...
    // the heap and still need the fork path below.)
    {
        ...
        if (access(up, F_OK) == 0) {
            ...
            const char* vd_exe = getenv("VALDROID_EXECUTABLE");
            const bool is_valheim = vd_exe && strcmp(vd_exe, "valheim.x86_64") == 0;
            if (!is_valheim) {
                setenv("RIMDROID_SDL_REMAP", "1", 1);
                LOGI("RIMDROID_SDL_REMAP=1: legacy SDL GL remap enabled");
            } else {
                unsetenv("RIMDROID_SDL_REMAP");
            }
