...
public class GameLauncher {
    ...
    public static void launch(GameInstance gameInstance) throws ErrnoException {
        ...
        // Empty libudev.so.1 (soname-registered by this preload) so SDL2's joystick/haptic init
        // succeeds; without it SDL_Init fails as a whole and Unity's window creation crashes.
        try {
            System.loadLibrary("vdudevstub");
        } catch (Throwable t) {
            Log.e(TAG, "udev stub load failed; SDL_Init may fail", t);
        }
        ...
        switch (gameInstance.settings().getTexTier()) {
            case com.valdroid.InstanceSettings.TEX_ULTRA:
                Os.setenv("RIMDROID_TEX_SHRINK", "2", true);
                Os.setenv("RIMDROID_TEX_DEEP_MIN", "2048", true);
                break;
            case com.valdroid.InstanceSettings.TEX_LOW:
                Os.setenv("RIMDROID_TEX_SHRINK", "2", true);
        ...
        Log.i(TAG, "Launching Valheim instance: " + gameInstance.getName());
        ...
        if (x11Test || gameInstance.getExecutableName().equals("valheim.x86_64")
                || new java.io.File(gameInstance.getGamePath(), "rd_x11").exists()) {
            ...
            if (!forceGlesZfa) {
                ...
                // SDL without binding anything to the ANativeWindow that Vulkan owns.
                Os.setenv("BOX64_LIBGL", AppStorage.requireSingleton().getLibraryPath() + "/libvdglxstub.so", true);
                // The Unity Linux player never pre-rotates: it always builds the swapchain with
                // preTransform=IDENTITY. On a portrait panel Android WSI reports ROTATE_90 and answers
                // every present with VK_SUBOPTIMAL_KHR, and Unity recreates the swapchain forever
                // (black screen). Report IDENTITY and swallow SUBOPTIMAL; the compositor rotates.
                Os.setenv("VALDROID_VK_FORCE_IDENTITY_TRANSFORM", "1", true);
            ...
            int visualId = com.valdroid.xserver.XServerRunner.getRootVisualId();
     ...
     */
    private static void setupMonoSymlink(com.valdroid.game.GameInstance gameInstance) {
        String monoDir = gameInstance.getGamePath() + "/" + gameInstance.getDataDirectoryName()
