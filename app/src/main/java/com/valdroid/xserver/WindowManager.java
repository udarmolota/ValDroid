...
import com.valdroid.xserver.errors.BadIdChoice;
import com.valdroid.xserver.errors.BadMatch;
import com.valdroid.xserver.errors.BadValue;
import com.valdroid.xserver.errors.XRequestError;
import com.valdroid.xserver.events.ConfigureNotify;
import com.valdroid.xserver.events.ConfigureRequest;
import com.valdroid.xserver.events.DestroyNotify;
import com.valdroid.xserver.events.Event;
import com.valdroid.xserver.events.Expose;
import com.valdroid.xserver.events.MapNotify;
import com.valdroid.xserver.events.MapRequest;
import com.valdroid.xserver.events.ResizeRequest;
import com.valdroid.xserver.events.UnmapNotify;
...
public class WindowManager extends XResourceManager {
    ...
    public void mapWindow(Window window) {
        ...
        if (!window.attributes.isMapped()) {
            ...
            if (!parent.hasEventListenerFor(Event.SUBSTRUCTURE_REDIRECT) || window.attributes.isOverrideRedirect()) {
                ...
                window.sendEvent(Event.VISIBILITY_CHANGE, new com.valdroid.xserver.events.VisibilityNotify(window));
    ...
    public void setFocus(Window focusedWindow, FocusRevertTo focusRevertTo) {
        ...
        if (old != null && old != rootWindow)
            old.sendEvent(Event.FOCUS_CHANGE, new com.valdroid.xserver.events.FocusOut(old));
        if (focusedWindow != null && focusedWindow != rootWindow)
            focusedWindow.sendEvent(Event.FOCUS_CHANGE, new com.valdroid.xserver.events.FocusIn(focusedWindow));
    }
    ...
    public void reparentWindow(Window window, Window newParent) {
        ...
        window.sendEvent(Event.STRUCTURE_NOTIFY,
                new com.valdroid.xserver.events.ReparentNotify(window, window, newParent));
        newParent.sendEvent(Event.SUBSTRUCTURE_NOTIFY,
                new com.valdroid.xserver.events.ReparentNotify(newParent, window, newParent));
        if (oldParent != null && oldParent != newParent)
            oldParent.sendEvent(Event.SUBSTRUCTURE_NOTIFY,
                    new com.valdroid.xserver.events.ReparentNotify(oldParent, window, newParent));
        if (wasMapped) {
            updateWmState(window, WM_STATE_NORMAL);
            window.sendEvent(Event.STRUCTURE_NOTIFY, new MapNotify(window, window));
