...
public class StickControlElement extends AbstractControlElement {
    ...
    int pointerId = -1;
    /**
     * Gamepad-stick gain, kept from ValDroid's own stick: a thumb on glass overshoots, so the
     * camera (right) stick ships at 70%. Stored in the description's sensitivity field, where
    ...
    public void setInputType(InputType inputType) {
        ArrayList<GLFWBinding> oldBindings = new ArrayList<>(this.bindings);
        clearBindings();
    ...
    private void dispatchEvent() {
        ...
        float nx = clamp(dx * SQRT_2 / r, -1.f, 1.f);
        float ny = clamp(dy * SQRT_2 / r, -1.f, 1.f);
        ...
        switch (this.inputType) {
            case MNK:
                ...
                break;
            case GAMEPAD:
                float gain = sensitivity / ControlElementDescription.DEFAULT_SENSITIVITY;
                nx = clamp(nx * gain, -1.f, 1.f);
                ny = clamp(ny * gain, -1.f, 1.f);
                switch (getBindingStick()) {
                    case LEFT_JOYSTICK: {
                        InputSink.sendJoystickAxis(GLFWBinding.GAMEPAD_AXIS_LX.code, nx);
                        InputSink.sendJoystickAxis(GLFWBinding.GAMEPAD_AXIS_LY.code, ny);
                        break;
                    }
                    case RIGHT_JOYSTICK: {
    ...
    public boolean handleMotionEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                // Zomdroid ignored CANCEL, so a system-cancelled touch (notification shade,
                // gesture nav) left the stick deflected and the character walking.
