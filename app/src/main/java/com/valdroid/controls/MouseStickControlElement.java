...
public class MouseStickControlElement extends AbstractControlElement {
    ...
    // The cursor itself lives in InputControlsView: one cursor shared by every mouse element,
    // the touchpad and a physical mouse, instead of one per element as in Zomdroid.
    ...
    @Override public void setScale(float value)                   { value = clamp(value, MIN_SCALE, MAX_SCALE); drawable.setScale(value); parentView.invalidate(); }
